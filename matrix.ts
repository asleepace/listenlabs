...
const CONFIG = {
  MIN_THRESHOLD: 0.35, // Higher = more strict
  TARGET_RANGE: 4_000, // Target number of people to view
  THRESHOLD_RAMP: 0.48, // Lower = More lenient early (slower ramp-up):
  URGENCY_MODIFIER: 3, // Higher is more urgent (was 2) 3=50% more than 2
};
  ...
  private calculateAdmissionScore(
    ...
    this.gameData.constraints.forEach((constraint) => {
      ...
      const targetProgress = Math.min(
        totalProcessed / CONFIG.TARGET_RANGE,
        1.0
      );
      ...
      this.gameData.constraints.forEach((otherConstraint) => {
        ...
        if (attributes[otherAttr] && otherNeeded > 0) {
          const correlation =
            this.gameData.attributeStatistics.correlations[attr]?.[otherAttr] ||
            0;
